...
import qs from "qs"
// import { reqInsertBankSingleChoiceQueInfoList } from '@/api/bankSingleChoiceQue'
export default {
  name: 'InfoUpload',
    ...
    handleSuccess({ results, header }) {
      ...
      results.forEach((item, index) => {
        ...
        Object.keys(item).forEach((key) => {
          if (key === '(必填)题目内容') {
            bankSingleChoiceQue.singleContent = (String)(item[key])
          }
          if (key === '(必填)选项A') {
            bankSingleChoiceQue.choiceA = item[key]
          }  if (key === '(必填)选项B') {
